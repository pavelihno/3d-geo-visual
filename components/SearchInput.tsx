...
const SearchInput: React.FC<SearchInputProps> = ({ label, placeholder, onSelect, value, isActive, onFocus, themeMode }) => {
    ...
    const timer = setTimeout(async () => {
      ...
      if (trimmedQuery.length >= MIN_QUERY_LENGTH && trimmedQuery !== value) {
        ...
        try {
          ...
          const message = err instanceof Error && err.message === 'RATE_LIMIT_EXCEEDED'
  ...
  return (
    <div className="relative w-full">
      ...
      <div className="relative group">
        <div className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${isActive ? 'text-blue-500' : `${theme.palette.textSecondary} group-focus-within:text-blue-400`}`}>
          <Search size={18} aria-hidden />
        </div>
        <input
          type="text"
          className={`w-full ${theme.palette.surface} border-2 ${isActive ? 'border-blue-500 ring-4 ring-blue-500/10' : `${theme.palette.borderStrong} group-hover:border-blue-300/80`} ${theme.textPrimary} pl-12 pr-4 py-4 ${designTokens.radii.lg} focus:ring-4 focus:ring-blue-500/15 focus:border-blue-500 outline-none transition-all ${designTokens.typography.body} placeholder:${theme.textSecondary} ${theme.focusRing}`}
      ...
      {error && (
        <div className="mt-2 flex items-start gap-2 text-xs text-amber-500" id={`${inputId}-error`} role="alert" aria-live="polite">
          ...
          {results.map((res, idx) => (
            ...
            >
              ...
              <div>
                ...
                <div className={`${designTokens.typography.subtle} uppercase tracking-wider ${theme.textSecondary}`}>
